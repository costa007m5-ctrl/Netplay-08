                ...
                <>
                  {movie.videoUrl || movie.videoUrl2 || (movie.type === 'series' && movie.episodes && movie.episodes.length > 0) ? (
                    ...
                    <motion.button 
                      ...
                      className="bg-purple-600/80 hover:bg-purple-600 text-white cursor-pointer px-6 md:px-12 py-2.5 md:py-6 rounded-md font-black uppercase tracking-widest flex items-center gap-2 md:gap-5 text-[10px] md:text-xl italic shadow-2xl transition-all"
                    >
                      <Sparkles size={14} className="md:w-8 md:h-8" /> 
                      <span className="whitespace-nowrap">Indicar ao Catálogo</span>
                    </motion.button>
                  )}
