...
const NetflixPlayer: React.FC<NetflixPlayerProps> = ({ 
  ...
  useEffect(() => {
    ...
    // Reset state
    setError(null);
    setIsLoading(false); // não mostramos overlay de cara, o autoplay vai tentar explodir o loading instantaneamente
    ...
    const initPlayer = () => {
      ...
      const initFreshMode = () => {
        ...
        setLoadingProgress(80); // Progresso inicial quase completo
        if (lowerSrc.includes('.m3u8')) {
            if (Hls.isSupported()) {
              const hls = new Hls({
                ...
                autoStartLoad: true,
                startLevel: 0,
                capLevelToPlayerSize: true, // Adicionado para priorizar start rápido mobile
                abrEwmaFastLive: 1,
                abrEwmaSlowLive: 3,
      ...
      const initResumeMode = () => {
        ...
        if (!isMounted || !video) return;
        setLoadingProgress(80); // Progresso inicial imediato
        if (lowerSrc.includes('.m3u8')) {
            if (Hls.isSupported()) {
              const hls = new Hls({
                enableWorker: true,
    ...
    const handleCanPlay = () => {
      setIsLoading(false);
      setLoadingProgress(100);
      setShowLogoOverlay(false); // removemos o overlay imediatamente
    ...
    const handleWaiting = () => {
      // Evitamos congelar a tela com overlay durante micropauses 
    };
    ...
    const handleProgress = () => {
      if (video.buffered.length > 0 && video.duration > 0) {
        ...
        // Se já temos qualquer buffer mínimo
  ...
  useEffect(() => {
    ...
    const lockOrientation = async () => {
      ...
      try {
        // Tentar entrar em fullscreen primeiro, pois alguns navegadores exigem isso para travar a orientação
  ...
  return (
    ...
    >
          ...
          >
            {backdropUrl && (
              <img 
                ...
                alt=""
                className={`w-full h-full object-cover transition-opacity duration-1000 ${logoUrl ? 'blur-[4px] opacity-40' : 'opacity-90'}`}
            ...
            {posterUrl && (
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none p-12 md:p-20">
                 <motion.img 
                   ...
                   alt=""
                   className={`h-[70%] md:h-[80%] rounded-2xl shadow-[0_0_60px_rgba(0,0,0,0.8)] border border-white/10 ${logoUrl ? 'opacity-50 blur-[2px]' : 'opacity-100'}`}
      ...
      {/* Overlay de Carregamento removido para velocidade máxima */}
      {isLoading && !error && showStuckButton && (
