  ...
  useEffect(() => {
    const fetchExternal = async () => {
      ...
      try {
        ...
        // Specific quick commands mapping
        ...
        // Deduplicate between External results
        const mergedExternal: Movie[] = [];
        filteredTmdb.forEach((tr: any) => {
            const currentKey = getMovieKey(tr);
            if (!mergedExternal.some(m => getMovieKey(m) === currentKey || m.id === tr.id)) {
                mergedExternal.push(tr);
            }
        });

        setExternalResults(mergedExternal);
    ...
    fetchExternal();
  }, [debouncedQuery, minRating, year, selectedGenres, myMovies]);
